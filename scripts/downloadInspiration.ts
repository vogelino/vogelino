...
async function downloadInspirations() {
	...
	const parsedInspirations = inspirationSchema.array().parse(existingInspirations)

	logSecondary(['📁✅ Creating the folders for the downloaded inspirations'])
	await createDirectoriesIfNotAlreadyThere('data')

	logSecondary([`💾💭 Saving the parsed inspiration along with the existing ones`])
	await writeJsonFile(INSPIRATIONS_JSON_PATH, [parsedInspiration, ...parsedInspirations])
