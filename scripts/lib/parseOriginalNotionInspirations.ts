...
function mapNotionInspirationLink(
  ...
  const { Name, URL, Date: inspirationDate, Tags } = rawLink.properties;
  ...
  const inspiration = {
    ...
    tags: Tags.multi_select.map((item) => item.name),
  };
...
export async function parseOriginalNotionInspirations(
  ...
  return Array.from(
    originalInspirations
      .map(mapNotionInspirationLink)
      .reduce((acc, cur) => {
        acc.set(cur.id, cur);
        return acc;
      }, new Map<string, MappedNotionInspirationLinkType>())
      .values()
  );
}
