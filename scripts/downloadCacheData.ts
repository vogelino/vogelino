...
import { getOriginalNotionTechnologies } from './lib/getOriginalNotionTechnologies'
...
const technologiesDatabaseId = process.env.NOTION_TECHNOLOGIES_DATABASE_ID || ''
...
async function downloadCacheData() {
	...
	const [
		...
		getOriginalNotionTechnologies(technologiesDatabaseId, notion),
	...
	await Promise.all([
		...
		writeJsonFile(ORIGINAL_TECHNOLOGIES_JSON_PATH, originalTechnologies),
	...
	logIndented(ORIGINAL_TECHNOLOGIES_JSON_PATH, 1)
	...
	logIndented(`🛠️ ${originalTechnologies.length} technologies`)
