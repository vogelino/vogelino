...
async function downloadCoolparsedCoolSitess() {
  ...
  const parsedCoolSitess = coolSiteSchema
    .array()
    .parse(existingCoolparsedCoolSitess);

  logSecondary(["📁✅ Creating the folders for the downloaded cool-sites"]);
  await createDirectoriesIfNotAlreadyThere("data");

  logSecondary([
    `💾💭 Saving the parsed cool-site along with the existing ones`,
  ]);
  await writeJsonFile(COOL_SITES_JSON_PATH, [
    parsedCoolSites,
    ...parsedCoolSitess,
  ]);
