...
export function getMediaShelfImages(slug: string) {
	const images = parseImagesForSlug(
		slug,
		import.meta.glob<AstroGlobImageType<'webp'>>('/src/assets/images/media-shelf/*.webp', {
			eager: true,
		})
	)
	return images[0]
...
export function getCoolSiteImages(slug: string) {
	const thumbnails = parseImagesForSlug(
		slug,
		import.meta.glob<AstroGlobImageType<'webp'>>('/src/assets/images/cool-sites/*.webp', {
			eager: true,
		})
	)
	const favicons = parseImagesForSlug(
		slug,
		import.meta.glob<AstroGlobImageType<'webp'>>('/src/assets/images/cool-sites-favicons/*.webp', {
			eager: true,
		})
	)
	return {
