...
export function getInspirationImages(slug: string) {
	const thumbnails = parseImagesForSlug(
		slug,
		import.meta.glob<AstroGlobImageType<'webp'>>('/src/assets/images/inspirations/*.webp', {
			eager: true,
		})
	)
	const favicons = parseImagesForSlug(
		...
		import.meta.glob<AstroGlobImageType<'webp'>>(
			'/src/assets/images/inspirations-favicons/*.webp',
			{ eager: true }
		)
	)
	return {
