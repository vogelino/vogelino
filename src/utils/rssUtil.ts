...
export async function createRSSFeed(
  ...
  const site =
    context.site?.toString().replace(/\/$/, "") || `https://vogelino.com`;
  ...
  const ogCoolSites = ogImages.find((file) =>
    file.url.includes("cool-sites")
  )?.image;
  ...
  return rss({
    ...
    items: [
      ...
      (a, b) =>
        (b.pubDate || new Date()).getTime() -
        (a.pubDate || new Date()).getTime()
    ),
  });
}
  ...
  return await Promise.all(
    coolSites.map(async (i) => {
      ...
      return {
        ...
        customData:
          i.thumbnail && `<webfeeds:cover image="${site}${i.thumbnail.src}" />`,
  ...
  return await Promise.all(
    projectsData.map(async (p) => {
      ...
      const imageString = ogImage
        ? `<img src="${site}${ogImage.src}" alt="${imageAlt}" />`
        : "";
      ...
      return {
        ...
        description: [
          stripMarkdown(`${p.data.description}.`),
          ogImage ? imageString : "",
        ].join(" "),
        ...
        categories: [
          ...
          ...p.data.technologies
            .map(
              (tech) =>
                technologies.find((t) => t.slug === tech.slug)?.data.name || ""
            )
            .filter(Boolean),
        ],
      } satisfies RSSFeedItem;
...
async function getHTMLContentBySlug(
...
async function getProjectHTMLContentBySlug(slug: string, site: string) {
  ...
  return [
    thumbnail &&
      `<img src="${site}${thumbnail.src}" alt="Thumbnail of the project ${p.title}" />`,
    ...
    renderHTMLList(
      ...
      images.map(
        (i, idx) =>
          `<img src="${site}${i.src}" alt="Project image ${idx + 1} for ${p.title}" />`
      )
    ),
      ...
      .map((key) =>
        renderHTMLList(
          ...
          p[key].map((coll: { slug: string }) => {
            ...
            return [
              col.url &&
                `<a href="${col.url}" title="More information about '${col.name}'">`,
              colImage &&
    ...
    renderHTMLList(
      ...
      p.technologies.map((tech) => {
        ...
        return [
          t.url &&
            `<a href="${t.url}" title="More information about '${t.name}'">`,
          tImage &&
